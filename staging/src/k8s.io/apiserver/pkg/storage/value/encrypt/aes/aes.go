...
// ctr implements encryption at rest of the provided values given a cipher.Block algorithm.
type ctr struct {
	block cipher.Block
}
...
func NewCBCTransformer(block cipher.Block) value.Transformer {
	return &ctr{block: block}
}

func (t *ctr) TransformFromStorage(data []byte, context value.Context) ([]byte, bool, error) {
...
func (t *ctr) TransformToStorage(data []byte, context value.Context) ([]byte, error) {
	blockSize := aes.BlockSize
	result := make([]byte, blockSize+len(data))
	iv := result[:blockSize]
	...
	stream := cipher.NewCTR(t.block, iv)
	stream.XORKeyStream(result[blockSize:], result[blockSize:])
	return result, nil
}
